...
for i in range(iter_number):
    ...
    if i%50 == 0 or i == (iter_number-1):
        try:
            ...
            pass
        # prediction_value = sess.run(prediction,feed_dict={xs:x_real})   # 原来的
        _, loss_now, prediction_value = sess.run([train_step, loss, prediction],feed_dict={xs:x_real,ys:y_real})
        # loss_now = tf.losses.mean_squared_error(y_real, prediction_value)   # compute cost
        # loss_now = tf.reduce_mean(tf.reduce_sum(tf.square(y_real-prediction_value),reduction_indices=[1]))  # 原来的
        print('[1 hidden layer:%d neure][%d]loss is: ' % (hidden_layer_1, i)  + str(loss_now))
        #画出预测
        lines = ax.plot(x_real,prediction_value,'r-',lw=2)
        handles, labels = plt.gca().get_legend_handles_labels()
        plt.legend(handles, labels = [ '[1-hidden-layer-%dneure] \n predicted data at iter = %d \n loss = ' % (hidden_layer_1, i) +  str(loss_now), 'origin data'], loc = 'best')
        plt.pause(1)
    if i == (iter_number-1):   # 
        plt.savefig('line-example-tensorflow-[1-hidden-layer-%dneure][%diter].png' % (hidden_layer_1, i) )
