...
path = 'Projects/Experiment/res/model-v1.2.2/Part3(loss=1)/%.3f/' % index
# 设置训练及测试数据路径
...
def train(x, y):
    # 实例化神经网络
    net = Net(n_feature=4, n_hidden1=10, n_hidden2=5, n_output=1)
    # Adam优化器
...
def draw_scatter(x_training, y_training, z_training, x_testing, y_testing, z_testing):
    ...
    plt.savefig(path + 'model-v1.2.0.png')
    plt.show()
...
def main():
    ...
    # 调用训练好的模型进行预测
    if not training_break:
        model_path = path + 'model-v1.2.0.pkl'
        # 此处不需要跟踪梯度
        with torch.no_grad():
