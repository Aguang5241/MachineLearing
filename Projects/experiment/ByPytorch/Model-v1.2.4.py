...
path = 'Projects/Experiment/res/model-v1.2.4/Part%d/%.3f/' % (output_num, index)
# 设置训练及测试数据路径
...
def train(x, y, i):
    if i == 1:
        loss_threashold_value = EL_loss_threashold_value
        y = y[:, 2].view(6, 1)
    elif i == 2:
        loss_threashold_value = UTS_YS_loss_threashold_value
        y = y[:, 0:2]
    else:
        print('Input i error!!!')
        training_break = True
    ...
    # 实例化神经网络
    net = Net(n_feature=4, n_hidden1=10, n_hidden2=5, n_output=i)
    # Adam优化器
    ...
    if not training_break:
        os.makedirs(path)
        torch.save(
            net, path + 'model-v1.2.4(i=%d).pkl' % i)
...
def draw_scatter(x_training, y_training, z_training, x_testing, y_testing, z_testing):
    ...
    plt.savefig(path + 'model-v1.2.4(i=%d-%.3f).png' % (output_num, np.random.randn(1)))
    plt.show()
...
def main():
    ...
    # print(y_list.shape)
    # (6, 3)
    training_break = train(x_standarded, y_list, output_num)

    # 调用训练好的模型进行预测
    if not training_break:
        model_path = path + 'model-v1.2.4(i=%d).pkl' % output_num
        # 此处不需要跟踪梯度
        with torch.no_grad():
